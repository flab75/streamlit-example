...
def _mock_listings(criteria: dict, source: str) -> List[Dict]:
    ...
    for i in range(count):
        ...
        listing_num = random.randint(2_000_000, 9_999_999)
        if source == "pap":
            url = f"https://www.pap.fr/annonce/vente-maisons-campagne-{listing_num}"
        else:
            url = f"https://www.leboncoin.fr/ad/immobilier/{listing_num}"
...
def _is_real_listing_url(url: str, base_search_url: str) -> bool:
    """Vérifie qu'une URL est une annonce individuelle, pas une page de recherche."""
    if not url or url == base_search_url:
        return False
    # Les annonces PAP finissent par un ID numérique : /annonce/vente-...-123456
    # Les pages de recherche finissent par un code région : g302, g75, etc.
...
async def _scrape_pap_async(criteria: dict) -> List[Dict]:
    ...
    try:
        async with async_playwright() as p:
            ...
            # PAP.fr utilise plusieurs structures selon la version — on essaie les 3
            selectors = [
                ".search-list-item",
                ".result-item",
                "article.item",
            ...
            for card in cards[:20]:
                try:
                    # Lien : cibler spécifiquement les liens d'annonces, pas navigation
                    link_el = await card.query_selector("a[href*='/annonce/vente']")
                    if not link_el:
                    ...
                    # Rejeter les liens de navigation / recherche / déjà vus
                    if not _is_real_listing_url(full_url, search_url):
                        continue
                    ...
                    title_el = await card.query_selector(
                        ".item-title, h2, h3, [class*='title']"
                    )
                    price_el = await card.query_selector(
                        ".item-price, [class*='price'], [class*='prix']"
                    )
                    location_el = await card.query_selector(
                        ".item-location, [class*='location'], [class*='ville']"
                    )
                    desc_el = await card.query_selector(
                        ".item-description, [class*='description'], p"
                    )

                    title = (await title_el.inner_text()).strip() if title_el else "Bien immobilier PAP"
                    price_text = (await price_el.inner_text()).strip() if price_el else "0"
                    location = (await location_el.inner_text()).strip() if location_el else criteria.get("ville", "France")
    ...
    try:
        async with async_playwright() as p:
            ...
            cards = await page.query_selector_all(
                "article[data-qa-id='aditem_container'], [data-test-id='ad'], article"
            )
            ...
            for card in cards[:20]:
                try:
                    # LeBonCoin : les liens d'annonces contiennent /ad/ ou /annonce/
                    link_el = await card.query_selector(
                        "a[href*='/ad/'], a[href*='/annonce/'], a[href]"
                    )
                    href = await link_el.get_attribute("href") if link_el else ""
                    full_url = f"https://www.leboncoin.fr{href}" if href.startswith("/") else href

                    # Une annonce LBC individuelle contient un ID numérique long
                    if not full_url or not re.search(r"/\d{8,}", full_url):
                        continue
                    if full_url in seen_urls:
                        continue
                    seen_urls.add(full_url)
                    ...
                    title_el = await card.query_selector(
                        "[data-qa-id='aditem_title'], h2, h3, [class*='title']"
                    )
                    price_el = await card.query_selector(
                        "[data-qa-id='aditem_price'], [class*='price'], [class*='prix']"
                    )
                    location_el = await card.query_selector(
                        "[data-qa-id='aditem_location'], [class*='location'], [class*='ville']"
                    )
                    desc_el = await card.query_selector(
                        "[data-qa-id='aditem_description'], p"
                    )

                    title = (await title_el.inner_text()).strip() if title_el else "Bien immobilier LBC"
                    price_text = (await price_el.inner_text()).strip() if price_el else "0"
                    location = (await location_el.inner_text()).strip() if location_el else criteria.get("ville", "France")
                    desc = (await desc_el.inner_text()).strip() if desc_el else ""
